...
* When `key` is `f` or `frequency`, the task's frequency is compared, and the search phrase is interpreted as an integer, representing the frequency to be searched.  A frequency of zero matches all the non-recurring tasks.  The operator `>` tests if the task has frequency at least as high as the specified frequency.  The operator `<` tests if the task has frequency at most as high as the specified frequency.  The operator `=` tests if the task's frequency is exactly equal to the specified frequency.  The convenience operator is an alias for `>`.
...
1. eXtensible Markup Language (XML) format: XML is a format that Deadline Manager is fully compatible with. If exported as XML format, tasks can be <<Importing a set of tasks: `import`, imported>> back into another copy of Deadline Manager.
...
****
* `csv`: If specified, Deadline Manager will export tasks in CSV format. Otherwise, it will export in XML format.
...
In Figure 2.2 above, when the user presses enter, Deadline Manager exports the file and display a success message.
...
[TIP]
You can overwrite the file by specifying r/overwrite. The command would be `export p/cs2101.txt /overwrite` in this case.
...
Imports tasks that were previously generated by an export command. Tasks will be retrieved from the provided `FILENAME` location.
