...
*Most of the time, the simplified filter syntax is sufficient to find the needed task(s) -- reading only the _Simplified filter syntax_ section is sufficient for most basic use cases.  We recommended that you read that section first and determine if you still need greater expressiveness, before reading the reset of the documentation contained in this guide.*
