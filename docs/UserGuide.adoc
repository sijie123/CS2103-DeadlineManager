...
* Parameters that contain spaces or `/` should be placed within quotes (either `'` or `"` are okay, but the opening and closing quotes should match) in order to prevent any ambiguity with other parameter prefixes.  (Deadlines need not be quoted because they do not contain any spaces and letters, and hence will never be interpreted as a parameter prefix.)  While it will work when not quoted as long as there is no ambiguity, this is not recommended as future versions of Deadline Manager may introduce new prefixes.
...
*Most of the time, the simplified filter syntax is sufficient to find the needed task(s) -- reading only the _Simplified filter syntax_ section is sufficient for most basic use cases.  We recommended that you read that section first and then determine if you still need greater expressiveness.*
...
* When `key` is `d`, `due` or `deadline`, the task's due date is compared, and the search phrase is interpreted as a date.  Dates must be in `d/m/y` format.  The operator `>` tests if the task's due date is on or after the specified due date.  The operator `<` tests if the task's due date is on or before the specified due date.  The operator `=` tests if the task's due date is exactly equal to the specified due date.  The convenience operator is an alias for `<`.
...
image::FilterUnitDiagramSetUG.png[width="200"]
