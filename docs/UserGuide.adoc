...
Sort comparators are the core of the `sort` command.  Each sort comparator specifies a particular attribute (e.g. deadline, priority) and a comparison direction, i.e ascending or descending.

The entire sorting command is composed of a chain of sort comparators which would be useful to determine which task to show earlier and latter +

****

What this means is that in case two tasks are in tie according to the first comparator, +
then the decision to place which task first is determined by the next comparator, and so on.
...
** When `key` is `p` or `priority`, the task's priority is compared.
...
earlier as compared to tasks with the tag `family`, finally followed by those tasks with the tag `cs2103t`

****
* In case a task does not belong to any of the tags mentioned it will be placed at the bottom of the sorted list.
* In case a task contains more than one tag specified by the user, then it will be attempted to be sorted according to those tags first which place it former in the sorted list.
****
...
* `resolve 1` + The 1st task displayed by the deadline manager will be deleted.
