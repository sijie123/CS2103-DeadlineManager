...
Step 1. The user enters an `add` command with the index of a task in the task list.
...
Step 7. The `Model` is updated: The `Model#addTask` method and the `Model#commitTaskCollection` method are invoked.

Step 8. A `CommandResult` object is returned.

TODO: KIEN: Add figures
...
Otherwise, `Model#updateTask` is invoked with a new `Task` where the deadline is updated.
