...
Step 4. A `AttachmentCommand` is constructed and initialized with the `AttachmentAction` constructed in Step 3.
...
Step 3. The `Model` is updated with the `Task` object returned by the `perform` method in Step 2.

Step 4. The `resultMessage` method of `AttachmentAction` is invoked. The returned value is used to construct a `CommandResult` object and returned to the `LogicManager`.
