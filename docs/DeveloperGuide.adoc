...
The above diagram shows the simplified sequence of operations to parse a filter expression into a predicate for non-set fields.  Take note of the following simplifications in the sequence diagram above:
...
3. (For the extended syntax only) The set operator, which describes the how the set of items (tags or attachments) in the identified field is compared to the set of items in the test phrase, when regarded as a set (more details below)
...
For set-based fields (e.g. tags), the `<`, `=`, `>` set operators represent _subset_, _equivalence_, and _superset_ relations respectively.  Each individual item is compared using the specified filter operator as specified in the previous _Filter operator_ section.
