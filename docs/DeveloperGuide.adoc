...
[NOTE]
As a more OOP model, we can store a `Tag` list in `deadline manager`, which `Person` can reference. This would allow `deadline manager` to only require one `Tag` object per unique `Tag`, instead of each `Person` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
...
. Fix `invalidAndValidPersonAddressBook.xml`, `typicalPersonsTaskCollection.xml`, `validAddressBook.xml` etc., such that the XML tests will not fail due to a missing `<remark>` element.
