...
* `VersionedAddressBook#commit()` -- Saves the current deadline manager state in its history.
* `VersionedAddressBook#undo()` -- Restores the previous deadline manager state from its history.
* `VersionedAddressBook#redo()` -- Restores a previously undone deadline manager state from its history.
...
The task manager supports import and export features. This feature uses 2 components: Storage and Model.
...
However, additional checks are performed to ensure that the user does not inadvertently overwrite the current working file,
or import and corrupt the current working file.
...
* importTaskCollection: Creates a new Import request event
* exportTaskCollection: Creates a new Export request event.

==== Inter-component communication
To facilitate communication between the two components, the deadline manager uses the event-driven model.
An import or export request event is first generated, then StorageManager will handle the event.
After data has been successfully read or written, another event is generated for Model to process.

Hence, StorageManager implements these additional methods to handle requests:
...
StorageManager passes the result of imports and errors with the following events:
...
Similarly, ModelManager implements these additional methods to handle requests:
...
Step 2. The ModelManager posts the ExportRequestEvent with the EventsCenter.
...
Step 2. The ModelManager posts the ImportRequestEvent with the EventsCenter.
...
The user can specify the deconflict algorithm with flags. By default, the ignore resolver is used.
