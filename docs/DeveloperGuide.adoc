...
The deadline manager supports a very flexible filtering system.  The filtering system can be utilized with the `filter` command.
...
There are three parts to parsing the filter operation:
...
2. A parser that splits each filter unit (e.g. `t:CS2101`) into its three constituent parts (e.g. `t`, `:`, `CS2101`) -- this is the lambda expression in the sequence diagram below
...
A tokenizer (this is the `StringTokenizer` class in the `parser` subdirectory) is shared by parts 1 and 2 to split the filter expression into tokens.
