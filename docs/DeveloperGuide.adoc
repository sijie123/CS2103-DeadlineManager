...
2. A parser that splits each filter unit (e.g. `t:CS2101`) into its three constituent parts if possible (e.g. `t`, `:`, `CS2101`) -- this is the lambda expression in the sequence diagram below, which resides in the `FilterCommandParser` class
...
[NOTE]
The second component in the list above also accepts a filter unit that contains the third part alone (e.g. `CS2101`) without the other two parts.  This is known as the _simplified filter unit syntax_, and will be explained in more detail below.
...
This is represented by the generic class `seedu.address.logic.parser.BooleanExpressionParser<T>`, and it is a general boolean expression parser that is designed to be unaware of the syntax of individual filter units.
...
There are two possible ways to express a filter unit:

1. The full syntax -- this allows for maximum customization of the filter unit
2. The simplified syntax -- this improves terseness and reduces cognitive overhead
...
As shown in the diagram above, a filter unit (using the full syntax) consists of three parts:
...
All textual and date fields are _eligible_.  This means that the name, deadline, and tags fields are tested against when the simplified syntax is used.  Numeric fields (i.e. priority and frequency) are omitted because they often inadvertently match a test phrase (e.g. when the user attempts to search for tasks which have a name or tag that contains numbers).

Each eligible field is compared as if using the convenience operator.  A match in _any_ eligible field of a task will cause the task to match this filter unit.  Errors in interpreting the test phrase in the context of a field are simply treated as a non-match of the field (e.g. the test phrase `Assignment` cannot be interpreted as a deadline, so the deadline field will be skipped when attempting to match this test phrase).

As with the full syntax, the simplified syntax supports quoting the test phrase if special characters are required.  A non-quoted test phrase only allows alphanumeric characters and the characters `_`, `-`, and `/`.  (The `/` character allows dates to work without quotes.)
