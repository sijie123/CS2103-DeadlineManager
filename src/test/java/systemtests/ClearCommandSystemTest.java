...
public class ClearCommandSystemTest extends TaskCollectionSystemTest {

    @Test
    public void clear() {
        final Model defaultModel = getModel();

        /* Case: clear non-empty deadline manager, command with leading spaces and trailing alphanumeric characters and
         * spaces -> cleared
         */
        assertCommandSuccess("   " + ClearCommand.COMMAND_WORD + " ab12   ");
        assertSelectedCardUnchanged();

        /* Case: undo clearing deadline manager -> original deadline manager restored */
        String command = UndoCommand.COMMAND_WORD;
        String expectedResultMessage = UndoCommand.MESSAGE_SUCCESS;
        assertCommandSuccess(command, expectedResultMessage, defaultModel);
        assertSelectedCardUnchanged();

        /* Case: redo clearing deadline manager -> cleared */
        ...
        /* Case: clear empty deadline manager -> cleared */
        assertCommandSuccess(ClearCommand.COMMAND_WORD);
        assertSelectedCardUnchanged();
     ...
     * result display box displays {@code ClearCommand#MESSAGE_SUCCESS} and the model related
     * components equal to an empty model. These verifications are done by {@code
     * TaskCollectionSystemTest#assertApplicationDisplaysExpected(String, String, Model)}.<br> Also
     * verifies that the command box has the default style class and the status bar's sync status
     * changes.
     *
    ...
    /**
     * Performs the same verification as {@code assertCommandSuccess(String)} except that the result
     * box displays {@code expectedResultMessage} and the model related components equal to {@code
     * expectedModel}.
     *
